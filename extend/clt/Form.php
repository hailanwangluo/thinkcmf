...
class Form{
    ...
    public function datetime($info,$value){
        ...
        $parseStr = '<input type="datetime" title="'.$name.'" name="'.$field.'" data-required="'.$info['required'].'" placeholder="请输入'.$name.'" value="'.$value.'" class="'.$info['class'].' layui-input" id="'.$info['class'].'">';
        if($info['required']==1){
            $parseStr .='</div>';
            $parseStr .='<div class="layui-form-mid layui-word-aux red">*必填';
        }
