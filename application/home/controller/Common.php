...
class Common extends Controller{
    ...
    public function initialize(){
        ...
        //获取多图方法
       // $duot = Db::name('product')->where('id',input('id'))->field('pics')->select();
       //   var_dump($duot);
      // foreach ($duot as $k => $v) {      
       //   $duot[$k]['images'] = explode(';',$v['pics']);
       //} 
          //var_dump($duot['images']);
        //  $this->assign('img',$duot);
        //获取多图方法
        // $duot = Db::name('product')->where('id',input('id'))->field('pics')->select();

        //  $duot['pics'] = explode(';',$duot['pics']);
 
        //   var_dump($duot['images']);
        
         //获取产品多图方法
        $duot = Db::name('product')->find(input('id'));

         $duot = explode(';',$duot['pics']);
        ...
        $this->assign('image',$thisCat['image']); //获取栏目图片
        ...
        //广告
          $adLista = cache('adLista');
        if(!$adLista){
            $adLista = Db::name('ad')->where(['type_id'=>5,'open'=>1])->order('sort asc')->limit('4')->select();
            cache('adLista', $adLista, 3600);
        }
        $this->assign('adLista', $adLista);
